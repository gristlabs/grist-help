...
Paste files and images using the keyboard shortcut or the "Paste" menu item in [attachment columns](https://support.getgrist.com/col-types/#attachment-columns). Note that browsers have certain limitations (e.g. Firefox only allows pasting a single file, while Chrome can do multiple).

![grist-core release](../images/newsletters/core-release.png)

Another month, another release. This one brings with it lots of new stuff beyond the above features, such as new API endpoints for enabling/disabling users, timeout environment vars, lots of accessibility updates and more! Check out the [full release notes for v1.7.5](https://github.com/gristlabs/grist-core/releases/tag/v1.7.5) for more!
