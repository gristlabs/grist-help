...
Recent versions, however, have enabled a JIT compiler that is known to
cause problems with Grist. This JIT compiler should be disabled for
Grist with the command-line argument `-c jit=off`. In a
`docker-compose.yaml` file, for example, this argument can be supplied
like this:
