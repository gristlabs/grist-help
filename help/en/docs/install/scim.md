...
!!! Note "About GRIST_SCIM_USER"
    While instance administrators (identified using the emails in `GRIST_DEFAULT_EMAIL` or `GRIST_SUPPORT_EMAIL`) can access the SCIM endpoints, it is a good practice to define an account dedicated let your identity provider use SCIM endpoints (following to the [principal of least access](https://en.wikipedia.org/wiki/Principle_of_least_privilege)).
