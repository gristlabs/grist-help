...
MCP-aware tool (such as Claude or ChatGPT) can use Grist's
MCP server to work with your team sites and documents: list and search tables, read and query rows,
add or update rows, and create new documents and tables.
...
can use it. Below we walk through Claude.ai and Claude Desktop, Claude Code in the terminal, and the
...
name and MCP URL pre-filled, so all you have to do is confirm the dialog and sign in.
...
categories. The tools act on your behalf, so they can only reach what your own account can reach and
can never do more than you could do yourself (and are further limited by the permissions you grant
on the consent screen, so you can narrow what the connector is allowed to do).
...
Yes. Add Grist as a [custom connector](#add-as-a-custom-connector) and use your own host in the
MCP URL: `https://<your-grist-host>/api/mcp`. The Claude directory listing connects to Grist SaaS
only.
...
<span
class="screenshot-full">*![claude-connector-settings](images/mcp/claude-connector-settings.png)*</span>
{: .screenshot-half }
...
First, check which Grist account you used when you connected Claude. If you are not sure, just ask
the AI something like "which Grist account am I logged in as?" and it will report the connected
account's name and email. If you have several Grist accounts (for example, a personal one and a work
