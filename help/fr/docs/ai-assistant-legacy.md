...
Assistant de Formule IA (Legacy)
==============
...
L'Assistant de Formule IA de Grist simplifie la partie la plus difficile des feuilles de calcul — les formules. L'assistant ne fait qu'une seule chose, écrire des formules en réponse à des requêtes en langage clair.
...
Aucune donnée n'est partagée avec OpenAI à moins qu'un utilisateur ne soumette une question à l'assistant. En savoir plus sur [l'utilisation des données](ai-assistant-legacy.md#politique-dutilisation-des-donnees).

## Comment Utiliser l'Assistant IA

Créez une [colonne de formule](formulas.md) et cliquez soit sur l'icône d'agrandissement, soit sur le lien texte "utiliser l'Assistant IA" dans la cellule.

<span class="screenshot-large">*![Ouverture de l'Assistant IA](images/ai-assistant-legacy/formula-cell-editor.png)*</span>
{: .screenshot-half }

Cela ouvrira un éditeur de formule étendu avec le chat de l'Assistant IA en dessous. Décrivez simplement ce que vous voulez que la formule fasse. [Voici quelques conseils.](ai-assistant-legacy.md#bonnes-pratiques)
...
<span class="screenshot-large">*![Assistant de Formule IA](images/ai-assistant-legacy/ai-assistant-dialog.png)*</span>
{: .screenshot-half }

## Assistant IA pour les Auto-hébergeurs

Pour les auto-hébergeurs souhaitant connecter leur instance Grist, définissez les [variables d'environnement liées à l'Assistant IA](https://github.com/gristlabs/grist-core#ai-formula-assistant-related-variables-all-optional){:target="\_blank"}.

Les variables ci-dessus permettent également l'utilisation d'autres modèles (y compris ceux qui peuvent être auto-hébergés). Voir [ces instructions](install/ai-assistant-legacy.md#utilisation-dopenrouter) pour plus d'informations.
...
* Parfois, l'assistant s'entête sur une mauvaise idée. S'il insiste sur une méthode de formule particulière, envisagez de supprimer la conversation et de recommencer. Cliquez sur le menu à trois points dans l'en-tête de l'Assistant IA pour effacer une conversation.

<span class="screenshot-large">*![Effacer la Conversation de l'Assistant IA](images/ai-assistant-legacy/clear-conversation.png)*</span>
{: .screenshot-half }
...
Votre requête et le schéma de votre document sont envoyés à [OpenAI](https://openai.com/){:target="\_blank"}. L'Assistant de Formule IA de Grist utilise le modèle gpt-4o, alias ChatGPT. La [Politique de Confidentialité](https://openai.com/api-data-privacy){:target="\_blank"} d'OpenAI décrit comment OpenAI gère vos données. La [Politique de Contenu](https://labs.openai.com/policies/content-policy){:target="\_blank"}, les [Politiques d'Utilisation](https://openai.com/policies/usage-policies){:target="\_blank"} et la [Politique de Partage et de Publication](https://openai.com/api/policies/sharing-publication/){:target="\_blank"} d'OpenAI décrivent comment l'Assistant IA de Grist et ses résultats peuvent être utilisés et partagés. Ceux qui violent les politiques d'OpenAI peuvent perdre l'accès à l'assistant IA de Grist.
