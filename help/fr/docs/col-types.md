...
"Appariements" vous permet d'ajouter des colonnes de données provenant de tables connexes. Vous pouvez utiliser des colonnes de référence pour relier des données dans différentes tables. En savoir plus sur [Références et Appariements](references-lookups.md).

*![Ajouter une colonne d'appariement](images/columns/columns-add-lookup.png)*
{: .screenshot-half }

"Raccourcis" liste les fonctions de [formule d'initialisation](formulas.md#trigger-formulas) les plus fréquemment utilisées.
...
- [Auteurs](formula-cheat-sheet.md#examples-of-automatic-date-time-and-author-stamps)
...
*![formatage des colonnes](images/columns/columns-formatting.png)*
{: .screenshot-half }
...
*![options de formatage des colonnes](images/columns/columns-formatting-options.png)*

Si vous souhaitez plus d'options de couleur, cliquez sur la boîte de couleur de remplissage puis trouvez la nuance que vous souhaitez. Si vous avez une couleur spécifique que vous souhaitez utiliser, vous pouvez entrer le code hexadécimal, les valeurs RGB ou HSL.

*![formatage de colonne hex](images/columns/column-formatting-hex.png)*
{: .screenshot-half }

Le style de cellule peut également être modifié en fonction de règles conditionnelles. En savoir plus sur [Formatage conditionnel](conditional-formatting.md).
...
Lorsque vous créez une nouvelle colonne, elle a initialement le type `Any`. Lorsque vous entrez la première cellule de la colonne, Grist essaie de restreindre ce type. Si vous entrez un nombre, la colonne sera changée en type `Numeric`, qui est aligné à droite par défaut. Si vous entrez quelque chose qui ne ressemble pas à un nombre, la colonne sera changée en type `Text`, qui est aligné à gauche par défaut.
...
Vous voudrez souvent contrôler le type de colonne manuellement. Vous pouvez le changer dans la section "Type de colonne". Par exemple, ici nous avons défini une colonne pleine de réponses "oui" et "non" pour être de type `Toggle` :

*![colonnes-toggle](images/columns/columns-toggle.png)*

Un avantage de le faire est que Grist peut maintenant vous offrir des moyens de visualiser la colonne qui sont spécialisés pour des valeurs de type on/off. Chaque type de colonne a différentes options dans la section "Format de cellule" du panneau latéral :

*![apparence des colonnes-toggle](images/columns/columns-toggle-appearance.png)*

Quel que soit le type de colonne, vous pouvez entrer **n'importe quelle valeur** dans les cellules. Si une valeur entrée est incompatible avec le type défini, la cellule sera mise en surbrillance avec une erreur (et les colonnes faisant référence à la valeur invalide afficheront également une erreur) :
...
DateHeure      | Date valide + heure.
Choix          | Une seule valeur d'une liste de valeurs valides prédéfinies.
...
Vous pouvez mettre n'importe quel texte que vous aimez dans ce type de colonne. Pour le formatage, vous pouvez contrôler l'alignement et le retour à la ligne, la couleur du texte et la couleur de fond.
...
- `https://getgrist.com` s'affichera comme <https://getgrist.com>.
- `Grist Labs https://getgrist.com` s'affichera comme [Grist Labs](https://getgrist.com) (liant à "https://getgrist.com" avec "Grist Labs" comme texte).
- `Email Help mailto:support@getgrist.com` s'affichera comme <a href="mailto:support@getgrist.com">Email Help</a>, un lien qui ouvrirait un programme de messagerie pour composer un e-mail à support@getgrist.com.

En général, la valeur jusqu'au dernier espace est utilisée comme texte de lien, tandis que le dernier mot est utilisé comme destination du lien.
...
Ce type est pour les nombres, y compris les nombres à virgule flottante. En plus de contrôler l'alignement et la couleur, vous pouvez choisir le format numérique, et le nombre minimum et maximum de chiffres à afficher après la virgule.
...
Les options sous FORMAT NUMÉRIQUE incluent :

- `$` : Format pour les montants en devises, tels que les dollars ou les euros. La sélection du `$` ajoutera un préfixe de devise, des séparateurs de milliers, et par défaut à 2 chiffres après la virgule. Cela ouvrira également un sélecteur de devises pour les devises internationales.
...
- `(-)` : Afficher les nombres négatifs entre parenthèses, sans signe moins devant. Cela est couramment utilisé en comptabilité, et généralement combiné avec les formats `$` ou `,`.

## Colonnes entières

Ceci est strictement pour les nombres entiers. Il a les mêmes options que le type numérique.
...
*![Format bascule](images/columns/columns-format-toggle.png)*
{: .screenshot-half }
...
Ce type est pour stocker des dates calendaires (sans composant horaire de la journée). Plus de détails dans [Travailler avec des dates](dates.md). Vous pouvez choisir le format pour les dates, voir la [référence de formatage des dates](https://momentjs.com/docs/#/displaying/format/).
...
Ce type est pour stocker des dates calendaires plus l'heure de la journée. Plus de détails dans [Travailler avec des dates](dates.md). Vous pouvez choisir le format pour les dates, voir la [référence de formatage des dates et heures](https://momentjs.com/docs/#/displaying/format/). Vous pouvez également spécifier le fuseau horaire à afficher.
...
Ce type est pour stocker une des valeurs valides d'un ensemble, où vous pouvez spécifier les valeurs disponibles.
...
L'éditeur de configuration prend en charge de nombreux raccourcis clavier pratiques. Vous pouvez appuyer sur les touches `Flèche haut` et `Flèche bas` pour naviguer entre les choix sélectionnés ; maintenir la touche `Maj` pour sélectionner plusieurs choix adjacents tout en cliquant ou en utilisant les touches fléchées ; et maintenir la touche `Command`/`Control` pour sélectionner plusieurs choix tout en cliquant. Pour sélectionner tous les choix, vous pouvez appuyer sur `Command`/`Control` + `A`.

L'annulation et la rétablissement sont également pris en charge dans l'éditeur de configuration. Lorsque votre curseur est en focus sur le champ de texte de l'éditeur, vous pouvez appuyer sur `Command`/`Control` + `Z` pour annuler votre dernière modification, et `Command`/`Control` + `Shift` + `Z` pour la rétablir.

L'éditeur de configuration prend également en charge le copier-coller. Pour copier, sélectionnez les choix que vous souhaitez copier et appuyez sur `Command`/`Control` + `C`. Pour coller, concentrez-vous sur le champ de texte et appuyez sur `Command`/`Control` + `V`. Les choix sont collés en bloc si le presse-papiers contient plusieurs lignes de texte. Les choix peuvent également être copiés depuis l'éditeur de configuration d'une colonne et collés dans une autre, ce qui copiera à la fois les valeurs et leurs couleurs configurées.

Lors de la saisie dans une cellule de colonne de choix, vos choix configurés seront affichés dans un menu de saisie semi-automatique. Vous pouvez soit cliquer sur un choix, soit utiliser les touches fléchées et `Entrée` pour ajouter un choix à une cellule. Si votre entrée n'est pas l'un des choix valides, Grist affichera une option de menu pour l'ajouter commodément comme choix valide et dans la cellule en une seule étape.
...
Ceci établit une référence croisée à une autre table. Vous pouvez spécifier la table à référencer et une colonne dans cette table à afficher. Il y a beaucoup de choses que vous pouvez faire avec ce type de colonne, voir [Colonnes de référence](col-refs.md) pour plus de détails.
...
Ce type de colonne vous permet d'insérer des fichiers et des images entiers dans les cellules. Lorsque des images sont ajoutées dans les cellules, une miniature d'aperçu est affichée dans la cellule. La barre "Taille" permet de contrôler l'échelle de cette miniature.
